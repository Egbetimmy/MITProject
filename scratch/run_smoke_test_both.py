...
def run_smoke_test_both():
    print("[Smoke] Starting Both (Predictive Gating) Smoke Test under constrained limits...")
    
    # 1. Configure gateway for Both (Mitigated)
    env_config = {
    ...
    # 5. Run k6
    k6_summary_file = os.path.join(BASE_DIR, r"scratch\smoke_both_k6_summary.json")
    ...
    print("\n=== CONTAINER CPU STATISTICS (BOTH SMOKE TEST) ===")
    ...
    if os.path.exists(k6_summary_file):
        ...
        print("\n=== K6 METRICS (BOTH SMOKE TEST) ===")
        print(f"Total Requests:      {total_reqs}")
        print(f"Orders Success Rate: {orders_rate * 100.0:.2f}%")
        print(f"Throttled count:     {throttled}")
