...
export function handleSummary(data) {
  const lines = [
    ...
    `orders_ok rate: ${(data.metrics.orders_ok && data.metrics.orders_ok.values) ? data.metrics.orders_ok.values.rate : 'n/a'}`,
    `products_ok rate: ${(data.metrics.products_ok && data.metrics.products_ok.values) ? data.metrics.products_ok.values.rate : 'n/a'}`,
    `throttled_429 count: ${(data.metrics.throttled_429 && data.metrics.throttled_429.values) ? data.metrics.throttled_429.values.count : 0}`,
    `http_req_duration p(99): ${(data.metrics.http_req_duration && data.metrics.http_req_duration.values) ? data.metrics.http_req_duration.values['p(99)'] : 'n/a'} ms`,
    '============================================\n',
  ];
  return {
    stdout: lines.join('\n'),
